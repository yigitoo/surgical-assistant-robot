...
class Endowrist():
    ...
    def _angle_command(self,angle_command_msg):
        ...
        """
        self.curr_angles = deepcopy(angle_command_msg.data)
        angle_in_deg =  np.rad2deg(self.curr_angles[0:3])
        motor_commands = self.rpy_to_motor_commands(roll = angle_in_deg[0], pitch = angle_in_deg[1], yaw = angle_in_deg[2], claw = angle_command_msg.data[3])
